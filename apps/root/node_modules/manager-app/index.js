var 
	 constatns = require('constants/index.js')
...
exports.list_spaces = function(req,res,callback) {
	var ret = {};
	ret.list = [];
	var data = fs.readdirSync(constatns.path_apps);  	

  	for(var i in data){
  		if(!(data[i] == 'node_modules') && !(data[i] == 'root')){
  			util.log('read path ('+data[i]+')');
  			var dataFile = fs.readFileSync(constatns.path_apps+data[i]+'/.info');
		  	var content_file = {};
...
exports.remove_space = function(req,res, callback) {
	try{
		exports.rmdir(constatns.path_apps+req.namespace+'/node_modules');
	}catch(e){
		util.error(e);
	}
	try{		
		exports.rmdir(constatns.path_apps+req.namespace);
		callback({iserror : false, msg : 'Removido com Sucesso'});
	}catch(e){
		util.error(e);
	}
};
...
exports.create_space = function(req,res, callback) {	
	fs.mkdir(constatns.path_apps+req.namespace, 0775, function() {
		exports.config_namespace(constatns.path_apps+req.namespace
			, req
			, function(ret) {
				callback(ret);
		});			
	...
	ret.msg = 'teste';
	 fs.readFile(path+'/.info', 'utf8', function (err,data) {
		...
		var content_index = "console.log('load "+content_file.namespace+" ....');\n\n"
			...
			+"\tres.end('App ("+content_file.namespace+")');\n\n"
			+"};\n\n"
