...
exports.list_spaces = function(req,res,callback) {
	...
	var ret = {};
	ret.list = [];
	var data = fs.readdirSync(constants.path_spaces);  	

  	for(var i in data){
  		if(!(data[i] == 'node_modules') && !(data[i] == 'root')){
  			util.log('read path ('+data[i]+')');
  			var dataFile = fs.readFileSync(constants.path_spaces+data[i]+'/.info');
		  	var content_file = {};
...
exports.remove_space = function(req,res, callback) {
	try{
		exports.rmdir(constants.path_spaces+req.namespace+'/node_modules');
	}catch(e){
		util.error(e);
	}
	try{		
		exports.rmdir(constants.path_spaces+req.namespace);
...
exports.create_space = function(req,res, callback) {	
	fs.mkdir(constants.path_spaces+req.namespace, 0775, function() {
		exports.config_namespace(constants.path_spaces+req.namespace
			, req
			, function(ret) {
				callback(ret);
		});			
...
exports.create_app = function(req,res, callback) {	
	fs.mkdir(constants.path_spaces+req.namespace+'/node_modules/'+req.nameapp, 0775, function() {
		exports.config_app(constants.path_spaces+req.namespace+'/node_modules/'+req.nameapp
			, req
			, function(ret) {
				callback(ret);
		});			
...
exports.config_app =  function(path, obj, callback) {
	var ret = {};
	ret.iserror = true;
	ret.msg = 'teste';
	ret.type = 'error';
	 fs.readFile(path+'/.info', 'utf8', function (err,data) {
		...
		content_file.nameapp = obj.nameapp;
		content_file.dtcad = new Date().getTime();
		...
		fs.writeFile(path+'/.info', JSON.stringify(content_file), function(err) {
			...
			if(err) {
				...
				ret.iserror = false;
				ret.type = 'success';
				ret.msg = "App criada com sucesso";												
			}					
		});

		var content_index = "console.log('load "+content_file.nameapp+" ....');\n\n"
			...
			+"exports.init = function(req, res){\n\n"
			+"\tres.end('<h1>Default Response [App] ("+content_file.nameapp+")</h1>');\n\n"
			+"};\n\n"
			+"console.log('load "+content_file.nameapp+" [ok]');";
		...
		fs.writeFile(path+'/index.js', content_index, function(err) {
			...
			if(err) {
				...
				util.log("Arquivo salvo");
				ret.iserror = false;
				ret.msg = "App criado com sucesso";
				ret.type = 'success';
				fs.mkdir(path+'/node_modules', 0775, function() {
					callback(ret);	
				});
...
exports.list_apps = function(req,res,callback) {

	var ret = {};
	ret.list = [];
	var path = constants.path_spaces+'/'+req.namespace+'/node_modules';
	var data = fs.readdirSync(path);  	

  	for(var i in data){  		
  			util.log('read apps ('+data[i]+')');
  			var dataFile = fs.readFileSync(path+'/'+data[i]+'/.info');
  			util.log('lendo aquivo .info da app ['+data[i]+'] -- '+dataFile);
		  	var content_file = {};
			if(!data || !JSON.parse(dataFile)){
				content_file = {};
			}else{
